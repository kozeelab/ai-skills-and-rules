...
1. 读取项目中的 skills/skill-auto-activator.md 文件
...
```
# 自动启动规则
每次新对话开始时，自动执行 skills/skill-auto-activator.md 中定义的冷启动流程。
```
...
```
              ...
              ▼                     ▼                     ▼
    ┌──────────────────┐  ┌─────────────────┐  ┌─────────────────┐
    │   📜 规则体系     │  │  ⚡ 自动化 Skill │  │  📦 按需 Skill   │
    │  (rules/)        │  │  (后台运行)      │  │  (手动触发)      │
    ├──────────────────┤  ├─────────────────┤  ├─────────────────┤
    │ • rule.md        │  │ • prompt-       │  │ • project-      │
    │ • api-design.md  │  │   optimizer     │  │   summary       │
    │ • database.md    │  │ • ai-rule-      │  │ • git-multi-env │
    │ • error-         │  │   generator     │  │ • awesome-      │
    │   handling.md    │  │ • code-review-  │  │   design        │
    │ • go/code-       │  │   auto-fix      │  │ • skill-creator │
    │   style.md       │  │ • skill-quality-│  │                 │
    │                  │  │   guardian      │  │                 │
    └──────────────────┘  └─────────────────┘  └─────────────────┘
         冷启动时                冷启动时              用户请求时
         全量加载                自动激活              按需加载
